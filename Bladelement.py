...
###Fast Req Methods:
    ...
    def RequestVelocities(self, vel_FreeFlow, vel_wing_translation, vel_wing_rotation, vel_AoA):
...
###Calculations:
    ...
    # self.LERT and self.chordD and the Velocities must be updated ahead
        ...
        # print('rawResultantFlow:', rawResultantFlow)
...
### Main implementations:
    def CopmputeAerodynamicForce(self):
        ...
        F_r_amp      = self.aDen * C_r * self.BladeElementswidth * \
                       np.abs(self.v_AoA) * np.matrix(self.y_data * self.y_data).transpose()
        # F_r_amp      = self.aDen * C_r * self.BladeElementswidth * \
        ...
        Sum_of_F_t_lift_amp    = np.sum(F_t_lift_amp)
        Sum_of_F_r_amp = np.sum(F_r_amp)
